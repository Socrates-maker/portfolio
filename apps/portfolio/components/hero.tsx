...
const BTN =
  "appearance-none cursor-pointer font-mono text-[13px] leading-none font-semibold tracking-[0.05em] uppercase px-[22px] py-3.5 rounded-[3px] inline-flex items-center gap-2.5 transition-[transform,background-color,color,border-color] duration-200 no-underline border-2 border-transparent group";
  ...
  return (
    ...
    >
      <div className="border border-hairline-strong bg-[color-mix(in_oklab,var(--color-bg)_96%,var(--color-cover)_4%)] relative hero-enter">
        <div className="grid grid-cols-[minmax(200px,280px)_1fr] gap-[clamp(24px,4vw,48px)] p-[clamp(24px,4vw,40px)] max-tablet:grid-cols-1">
          <div>
            ...
            <div className="font-mono text-[9px] tracking-[0.16em] uppercase text-fg-soft text-center mt-2 max-tablet:w-[148px] max-tablet:mx-auto">
              {data.hero.photoCaption}
            </div>
          </div>

          <div className="flex flex-col">
            <div className={FIELD_ROW}>
              <span className={FIELD_LABEL}>{data.ui.fieldName}</span>
              <h1
                className={`${FIELD_VALUE} text-[clamp(30px,4.4vw,52px)] leading-[1.02] tracking-[-0.01em] text-fg`}
              >
                {data.hero.nameFirst} <em className="italic text-stamp">{data.hero.nameLast}</em>
              </h1>
            </div>

            <div className={FIELD_ROW}>
              <span className={FIELD_LABEL}>{data.ui.fieldProfession}</span>
              <p className="font-sans font-medium text-[clamp(15px,1.6vw,18px)] text-fg-muted">
                ...
                <span className="inline-block whitespace-nowrap font-mono font-normal text-[0.75em] bg-gold/20 border border-gold px-2 py-0.5 rounded-sm ml-2 uppercase tracking-[0.04em]">
                  {data.hero.roleAccent}
                </span>
              </p>
            </div>

            <div className={FIELD_ROW}>
              <span className={FIELD_LABEL}>{data.ui.fieldLocation}</span>
            ...
            <div className={FIELD_ROW}>
              <span className={FIELD_LABEL}>{data.ui.fieldRemarks}</span>
            ...
            <div className={`${FIELD_ROW} border-b-0 pb-0`}>
              <span className={FIELD_LABEL}>{data.ui.fieldPriorField}</span>
        ...
        <div className="flex items-center gap-3.5 flex-wrap px-[clamp(24px,4vw,40px)] py-[clamp(16px,3vw,24px)] border-t border-dashed border-hairline-strong">
        ...
        <div className="px-[clamp(24px,4vw,40px)] pb-[clamp(28px,4vw,40px)]">
          <div className="flex gap-3 mt-7 flex-wrap">
            ...
            <a
              href="#work"
              className={`${BTN} bg-transparent text-fg border-fg hover:bg-fg/6 focus-visible:outline focus-visible:outline-2 focus-visible:outline-gold focus-visible:outline-offset-2`}
            >
